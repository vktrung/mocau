...
func (biz *updateBusiness) UpdateProduct(ctx context.Context, id int, data *model.ProductUpdate) error {
    // Chỉ lấy thông tin sản phẩm hiện tại nếu cần xóa ảnh cũ
    var currentImage *common.Image
    if data.Image != nil {
    ...
    // Xóa ảnh cũ sau khi update thành công (để tránh mất dữ liệu nếu update fail)
    if data.Image != nil && currentImage != nil {
        if err := upload.DeleteImageFromProduct(currentImage); err != nil {
